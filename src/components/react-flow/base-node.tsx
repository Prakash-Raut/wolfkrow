...
export const BaseNode = forwardRef<HTMLDivElement, BaseNodeProps>(
  ({ className, status, ...props }, ref) => (
    ...
    >
      {props.children}
      {status === "error" && (
        <XCircleIcon className="size-2 absolute right-0.5 bottom-0.5 text-rose-700 stroke-3" />
      )}
      {status === "success" && (
        <CheckCircle2Icon className="size-2 absolute right-0.5 bottom-0.5 text-emerald-700 stroke-3" />
      )}
      {status === "loading" && (
        <Loader2Icon className="size-2 absolute -right-0.5 -bottom-0.5 text-sky-700 stroke-3 animate-spin" />
      )}
    </div>
  ),
);
